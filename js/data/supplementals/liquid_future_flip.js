...
// "Future (1st sg.)" so the renderer prints "click to reveal future →". Both
// faces also print the verbal (future) stem after a comma — the same stem on
// each side — so the pair is anchored to the stem that connects them (e.g.
// μένω, μεν- ↔ μενῶ, μεν-). keyVerb flags the high-frequency verbs.
//
// Mounce introduces the liquid future in ch 20 (Part 3 — Indicative Verb), so
// these are tagged with week: 3 for grouping under the Part III supplemental
// section in the chapter selector, alongside the second-aorist flip set.
...
(function () {
  ...
  function entry(present, presentGloss, future, futureGloss, stem, note, keyVerb) {
    return {
      stemFlip: true,
      stemFlipAorist: 'Future (1st sg.)',   // back-face label override
      g: present,
      e: presentGloss,
  ...
  window.registerSupplementalVocabSet('W3_LIQUID_FUTURE_FLIP', {
