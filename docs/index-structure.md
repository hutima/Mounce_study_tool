...
| Lines      | id                            | Purpose |
|-----------:|-------------------------------|---------|
|  ~208      | `#contactAuthorOverlay`       | "Contact the author" credit modal — maintainer bio, blog/LinkedIn links, an "Other projects" list (`.contact-projects`), and donation info (e-transfer `t.hutama@queensu.ca` / Venmo). Opened by the `.app-footer` link (`openContactAuthorModal()`); closed via `closeContactAuthorModal()` / Esc. Ported from duff #323. |
