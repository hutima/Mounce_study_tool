...
|  164–166   | `#paradigmFocusRowPrimary`                    | Focused-paradigm dropdown (`#paradigmFocusSelectPrimary`) — hidden unless in a mode that uses it. Options come from `listAvailableParadigmsByCategory` (`js/domain/grammar/paradigm_focus.js`). Verbs Mounce splits across many principal-part lemma keys (λύω → 15, δίδωμι → 2) also get a summative **"<base> — all forms"** aggregate at the head of their category: a virtual lemma that pools every member's chapter-gated cards into one deck (and the full optional paradigm when the optional-extensions toggle is on). Membership is driven by `window.PARADIGM_VARIANT_FAMILIES` in `lemma_inventory.js`. Hidden **only** while `#parsingCustomReviewToggle` is on (the checklist replaces it); for `#parsingShuffleAllToggle` it **stays visible** with a head **"All paradigms through selected chapter"** sentinel (`PARSING_SHUFFLE_ALL_VALUE` in `navigation.js`) as the toggle's dropdown face — picking it turns shuffle-all on, picking a real paradigm turns it back off (`setMorphFocusedParadigm` / `syncParadigmFocusUi` keep the two in sync). The sentinel is suppressed in Build mode (Lookup), where the dropdown is a single-paradigm reference. Each category optgroup with ≥2 concrete lemmas also leads with a "↯ Shuffle all — <type>" pick (a `__shuffleType__:` sentinel that pools the whole category via `getCardsForParadigmCategory`). |
...
|  ~204      | `<footer class="app-footer">`                 | Small centered **"Contact author"** credit link (`.app-footer-link` → `openContactAuthorModal()`), last child of `.app` after the review shell. Opens `#contactAuthorOverlay`. |
...
| Lines      | id                            | Purpose |
|-----------:|-------------------------------|---------|
|  ~208      | `#contactAuthorOverlay`       | "Contact the author" credit modal — maintainer bio, blog/LinkedIn links, an "Other projects" list (`.contact-projects`), and donation info (e-transfer `t.hutama@queensu.ca` / Venmo). Opened by the `.app-footer` link (`openContactAuthorModal()`); closed via `closeContactAuthorModal()` / Esc. Ported from duff #323. Every external link here carries `onclick="return openExternalLink(this, event)"` (`main.js`) — a bare `<a target="_blank">` is dead in an installed PWA (iOS swallows the new-tab default, and `touchTapBridge.js` routed the tap to the overlay's close handler since a link is not a tap target); the onclick makes each link a tap target and opens it via `window.open()` (mailto/tel via `location.href`) from inside the gesture. |
...
|  480–545   | `#shortcutsOverlay`           | User guide. Dismiss via the bottom Close, a top-right ✕ (`.modal-close-x`), Esc, or a backdrop tap (`onclick` closes when `event.target===this`). Footer (`.user-guide-actions`, `space-between`) has a **Contact author** link (`openContactAuthorModal()`) on the left, and a right-hand `.user-guide-actions-right` group with an **Install app** button (`#userGuideInstallBtn` → `triggerInstall()`, hidden once the app is already installed) and the **Close** button. The Contact-author modal opens *on top* (it sits at `z-index:1100`). **No changelog yet** — Mounce is not released. The duff equivalent embeds a `<details class="user-guide-changelog">` here; once Mounce ships, add one. |
|  ~547      | `#installInstructionsOverlay` | **"How to install (PWA)" modal** — platform-aware (iOS Safari / Android Chrome / generic) numbered steps for adding the app to the Home Screen. Body (`#installInstructionsBody`) is filled by `openInstallInstructions()` in `js/ui/pwaInstall.js`. Two footer actions: **Got it!** (`closeInstallInstructions()`) and **Don't show again** (`dontShowInstallAgain()` — the explicit forever-dismiss). Opened from the top install banner's **Install** button (when there's no native prompt to fire) and from the user-guide **Install app** button (`#userGuideInstallBtn`). See "PWA install prompt" below. |
...
## PWA install prompt (`js/ui/pwaInstall.js`)

A "install to Home Screen" nudge, modelled on the achievement toast but with
explicit dismissal. Self-contained module; `main.js` calls `initPwaInstall()` +
`maybeScheduleInstallPrompt()` once at startup (right after
`initializeConsentGate()`), and the user-guide button calls `triggerInstall()`.

- **Top banner** (`#pwaInstallHost`, created on demand) — a persistent card:
  download icon, "Get the best experience", body copy, an **Install** button and
  a **✕**. Styled like `.level-toast` (`.pwa-install*` in `styles.css`) but it
  **never auto-hides** — only Install or ✕ clears it. `z-index:950` (below the
  `.consent-overlay` 1000) so the how-to modal's dimmed backdrop sits on top.
- **Shown once, phones only.** `maybeScheduleInstallPrompt()` arms a one-time
  ~60 s timer; `tryShowScheduled()` re-arms while any `.consent-overlay.show` is
  open (so it lands *after* the consent gate). Gated on `isLikelyPhone()`
  (iPhone/Android-Mobile UA, or coarse-pointer + short-edge ≤480px) — desktop is
  a no-op. Skipped when already `isStandalone()`.
- **Never reappears after dismissal.** The banner ✕ and the modal's **Don't show
  again** both set `localStorage['mounceBbgFlashcardsInstallPromptDismissedV1']`
  (string literal in the module, *not* a `store.js` export — avoids the
  cross-module new-export SW-update hazard). The auto-show respects the flag; the
  user-guide button passes `{force:true}` to re-surface regardless.
- **Install action** (`triggerInstall`) — fires the captured Chromium
  `beforeinstallprompt` if present (Android), else opens
  `#installInstructionsOverlay` with platform-detected steps (iOS Safari /
  Android Chrome / generic). `appinstalled` marks dismissed + tears everything
  down. The module is **precached in `sw.js`** like the other `js/ui/*` modules.
