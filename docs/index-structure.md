...
|  480–545   | `#shortcutsOverlay`           | User guide. Dismiss via the bottom Close, a top-right ✕ (`.modal-close-x`), Esc, or a backdrop tap (`onclick` closes when `event.target===this`). Footer (`.user-guide-actions`, `space-between`) has a **Contact author** link (`openContactAuthorModal()`) on the left that opens `#contactAuthorOverlay` *on top* (it sits at `z-index:1100`). **No changelog yet** — Mounce is not released. The duff equivalent embeds a `<details class="user-guide-changelog">` here; once Mounce ships, add one. |
