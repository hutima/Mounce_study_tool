...
|  166–171   | `#cardArea`                                   | **Main flashcard mount.** Contains a placeholder `.empty-state`; JS replaces it. In parsing mode the step-by-step walk renders here: the current step's MC choices, the "I don't know" / "I give up" footer, and (once at least one guess exists) an "↶ Undo guess" button (`renderMorphUndoRow` in `render.js` → `undoMorphologyStep` in `main.js`). Undo is a **pedagogical step-back, not a free retry** — it re-opens the previous dimension to re-pick, but every undone dim is added to `morphStepState.forcedWrong` and scores **half credit** (0.5: re-picked right) or 0 in the finalized attempt, so it never reaches the full 1 a clean pick earns and always fails the 2/2 "known" gate (`evaluateRecentAttempt` requires exactly 1). The walk handlers (`answer`/`skip`/`giveUp`) push a pre-action snapshot onto `morphStepState.history`; undo pops it and force-fails the dims it reverts. Undo is shown only during the walk, never on the completed parse summary; a reattempted dim re-picked correctly reads amber with a `*` asterisk pointing to a shared "… reattempted — counts as a miss" footnote (red ✗ if still wrong). The summary's bottom meta line now **names the paradigm** being assessed — "Paradigm: &lt;lemma&gt; — &lt;category&gt;" via `paradigmCategoryForLemma` (`paradigm_focus.js`), surfaced post-parse so it can't leak the answer the way an on-card source label would. Parsing stays off the record: none of the undo/step handlers call `noteStudyInteraction()`. |
